...
const REVIEWS: Review[] = [
  ...
  },
  {
    ...
    rating: 5,
    text:
      "Best instructor I’ve had. Pre‑test session covered exactly what the examiner looked for on the day.",
  },
  {
    name: "Aoife K.",
    date: "2025-06-02",
  ...
  },
  {
    ...
    rating: 5,
    text:
      "Knows the Churchtown and Dún Laoghaire routes inside out. Tips were spot on. Highly recommend.",
  },
  {
    name: "Laura F.",
    date: "2025-05-01",
    rating: 5,
    text:
      "Booked a refresher before my test — invaluable. Clear coaching and zero waffle.",
  },
  {
    name: "Mark S.",
    date: "2025-04-15",
    ...
    <div className={`flex ${size}`} aria-label={`${rating.toFixed(1)} out of 5 stars`}>
      {Array.from({ length: 5 }).map((_, i) => (
        <svg
          key={i}
          viewBox="0 0 24 24"
          className={`h-6 w-6 ${i < full ? "fill-red-600" : "fill-gray-200"}`}
          aria-hidden="true"
        >
      ...
      <div className="space-y-3 text-center">
        ...
        <div className="flex items-center justify-center gap-3">
          ...
          <div className="text-lg font-semibold text-gray-900">
            <span className="text-red-600">5.0</span> out of 5
            <span className="text-gray-500"> — based on {REVIEWS.length} reviews</span>
          </div>
        </div>
        <p className="text-gray-700 max-w-2xl mx-auto">
          Real feedback from learners across Dublin — Tallaght, Dún Laoghaire, and the old
          Churchtown routes.
        </p>
      </div>
